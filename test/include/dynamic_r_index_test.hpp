            ...
            */
                ...
                else
                {
                    for (uint64_t i = 0; i < collect_sa.size(); i++)
                    {
                        if (sampling_sa[i] != UINT64_MAX && collect_sa[i] != sampling_sa[i])
                        {
                            ...
                            {
                                std::cout << std::endl;
                                stool::DebugPrinter::print_integers(collect_sa, "SA(collect)");
                                stool::DebugPrinter::print_integers(sampling_sa, "SA(test)   ");
                            }
            ...
            static void save_and_load_test(uint64_t text_size, uint8_t alphabet_type, uint64_t seed)
            {
...

                {
                    ...
                    stool::dynamic_r_index::DynamicRIndex::store_to_file(drfmi, os, stool::Message::NO_MESSAGE);
                }
                ...
                stool::dynamic_r_index::DynamicRIndex drfmi2;
                {
                    ...
                    auto tmp = stool::dynamic_r_index::DynamicRIndex::load_from_file(ifs, stool::Message::NO_MESSAGE);
                    drfmi2.swap(tmp);
                }
