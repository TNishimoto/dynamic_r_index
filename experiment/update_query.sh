...
for item in "${list[@]}" ; do
  set -- $item
  echo "Update $1"
  nohup /usr/bin/time -f "#update, DR-index, _, $1, pattern_length, 1, Time(sec), %e, Memory(KB), %M" ./query.out -i ${folderpath}/$1.drfmi -q ${querypath}/$1.update.1.tsv -w ${logpath}/$1.r.update.1.log >> ${logpath}/update_r.1.log
done
